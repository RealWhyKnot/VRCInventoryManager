...
function Get-ExpectedReleaseRevision {
    param([string] $DateStamp, [string] $ExcludeTag)
...
try {
    if ($Tag -notmatch "^v(\d{4})\.(\d+)\.(\d+)\.(\d+)(-[A-Za-z0-9][A-Za-z0-9.-]*)?$") {
        throw "Release tag must be vYYYY.M.D.N or vYYYY.M.D.N-suffix, got '$Tag'."
    }
    ...
    if ($actualRevision -ne $expectedRevision) {
        throw "Release tag $Tag uses revision $actualRevision, expected $expectedRevision for $dateStamp. Use .0 when no same-day tag exists; otherwise increment the highest same-day release or prerelease revision by one."
    }
